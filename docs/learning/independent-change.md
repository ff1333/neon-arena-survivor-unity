...
1. 扩大为有边界的大地图，相机跟随玩家；敌人在地图内随机选择出生点，先显示 1.5 秒红色 `×` 预警。
...
- [x] 场景中只有一个敌人时，行为与旧版本一致。
- [x] 两个敌人都在最近距离带内时，优先攻击血量更低者。
- [x] 低血量敌人在距离带外时，优先攻击更近的敌人。
- [x] 血量相同的两个候选中，优先攻击更近者。
- [x] 没有敌人时不生成子弹，Console 无红色错误。
...
- [x] 敌人首次碰到玩家时只扣一次固定血量并立即消失。
- [x] 一只碰撞自毁的敌人不增加 Kills，也不生成经验物。
- [x] 敌人从池中再次生成后仍能正常造成一次伤害。
- [x] 同一物理帧出现重复触发时，玩家不会被同一敌人扣两次血。
- [x] 子弹击杀仍然掉经验并增加击杀，不受该修改影响。
...
- [x] 0、60、120 秒的实际生成趋势与公式一致。
- [x] 足够长时间后间隔保持在 minimumInterval，不继续降低。
- [x] 难度变化是连续的，不在 45/90 秒突然成倍跳升。
- [x] 连续运行 6 分钟没有无限刷怪、卡死或 Console 红色错误。
...
- [x] 初始攻击距离外、但仍在画面内的敌人不会被攻击。
- [x] 选择一次 AttackRange 后，新增加的距离范围内敌人可以成为目标。
- [x] 攻击距离达到上限后，连续多次升级都不再出现 AttackRange。
- [x] 大地图中即使界外敌人处于攻击距离内，也不会被自动锁定。
- [x] Level 2、4、5 时三选一中不会出现增加弹道。
- [x] Level 3、6、9 等节点必定出现一个增加弹道选项，且三个选项仍不重复。
- [x] 1 到 5 弹道均围绕目标方向左右对称。
- [x] 达到 5 弹道后，连续多次升级都不再出现该选项。
- [x] 候选过滤后仍至少有三个合法升级，不会导致数组越界。
- [x] 多弹道下对象池不重复释放，Console 无红色错误。
...
作为玩家，我希望在比单屏更大的有限场地中走位，由相机平滑跟随；敌人在地图内随机出生，并通过红色 `×` 提前提示即将出现的位置。
...
4. 敌人出生点在世界边界内随机选择，不要求位于相机视口外。
5. 选择出生点时必须与玩家至少相距 5 个世界单位，不设置最大距离。
6. 只有进入视口且在武器射程内的敌人才参与索敌。
7. Projectile 继续离开视口即回池。
8. 地面使用 40×24 的深灰色 Square Sprite，并用六条参考线显示相机移动。
9. 出生点先显示红色 `×` 1.5 秒，随后预警消失，敌人在原位置出现。
10. 预警倒计时使用游戏时间，暂停游戏时倒计时也暂停。

### 全地图随机出生规则

本轮明确采用全地图随机出生。先在 ArenaBounds 内随机采样，并拒绝距玩家小于 5 个单位的位置；连续 64 次都不合法时，使用距玩家最远的地图内角点。敌人出现前通过 1.5 秒红色 `×` 预警给玩家反应时间。

### 新增或修改的内容

- 新增 `CameraFollow.cs`：LateUpdate 平滑跟随并限制相机中心。
- 新增 `EnemySpawnWarning.cs` 和 `SpawnWarning.prefab`：显示预警并在倒计时结束后从对象池生成敌人。
- 新增 `SpawnWarningPool`：复用预警对象，避免每次刷怪 Instantiate/Destroy。
- 新增或集中定义世界边界数据，避免 Player、Camera、Spawner 各写一套不一致的数值。
- `PlayerMovement.cs`：由相机边界 Clamp 改为世界边界 Clamp。
- `EnemySpawner.cs`：在地图内随机采样，检查与玩家至少相距 5 个单位，然后生成预警对象。
...
- [x] Player 能离开初始画面，相机平滑跟随。
- [x] Player 到世界四边后无法越界，相机也不会显示地图外区域。
- [x] 出生点只在地图内随机选择，选择时与 Player 至少相距 5 个单位。
- [x] 红色 `×` 显示 1.5 秒后消失，敌人在原位置出现。
- [x] 暂停游戏时，预警倒计时也暂停。
- [x] 界外敌人不会被自动锁定；进入画面后才可能成为目标。
- [x] 子弹离开画面后回池，不会在界外杀敌。
- [x] 经验物全部位于地图内，并能通过移动重新拾取。
- [x] 16:9、窄屏和宽屏下相机边界均正确。
- [x] 连续运行 6 分钟，场外没有永久滞留的敌人，Console 无红色错误。
...
2. 改为地图内随机选择出生点、至少距玩家 5 单位，并增加 1.5 秒红色 `×` 预警：`feat: telegraph random enemy spawns`。
